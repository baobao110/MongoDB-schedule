...
public class ChooseAct extends Activity {
	...
	private ImageView bookimg,lximg,pkimg;
	...
	protected void onResume(){ 
		...
		speaker.startSpeaking("您可以选择奥运知识学习，奥运竞答练习，也可以选择奥运知识Pk。",synthesizerListener);
	}
  ...
  //语音合成监听器
    ...
    protected void word_check(String str) {
    	...
    	if(str.contains("退出")){
			...
			intent.putExtra("type", "2"); 
			startActivity(intent);
		}else if(str.contains("pk") || str.contains("PK")){
			if(recognizer.isListening()){
				recognizer.stopListening();
			}
			if(speaker.isSpeaking()){
			...
			speaker.startSpeaking("请告诉小奥，您是选择奥运知识学习，奥运竞答练习，还是选择奥运知识PK。",synthesizerListener);
		}
	}
